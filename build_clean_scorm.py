#!/usr/bin/env python3
"""
Build clean SCORM 1.2 packages: one zip per slide/topic.
Each package contains ONLY:
  - Slide content (bilingual EN/AR)
  - Interactive activities (match, flashcard, drag-drop) where applicable
  - Base64-embedded images
  - SCORM 1.2 API integration
  
Excluded: videos, quizzes, topic menus/sidebars
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, 'scorm-per-slide-clean')
...
# ══════════════════════════════════════════════════════════════
# INTERACTIVE ACTIVITIES - keyed by activity id
# Each activity will be placed on the most relevant slide
# ══════════════════════════════════════════════════════════════
...
# Map: which activity goes on which slide (slide_id -> [activity_ids])
# Place activities on the most relevant content slides
SLIDE_ACTIVITIES = {
    # Module 1: match on trad-vs-ai slide, flashcards on types slide
    'm1-s4': ['s1-match'],      # Traditional vs AI slide -> match activity
    'm1-s5': ['s1-flash'],      # Three Types slide -> flashcard activity
    # Module 2: drag-drop on building blocks, match on outputs
    'm2-s7': ['s2-drag'],       # Building Blocks Together -> drag-drop
    'm2-s6': ['s2-match'],      # Outputs slide -> match building blocks
    # Module 3: flashcards on agent vs standard, match on capabilities
    'm3-s3': ['s3-flash'],      # AI Agents vs Standard -> flashcards
    'm3-s11': ['s3-match'],     # Where AI Agents Add Value -> match capabilities
    # Module 4: drag-drop on describe-generate-refine, flashcards on tools
    'm4-s3': ['s4-drag'],       # Describe-Generate-Refine -> drag-drop
    'm4-s7': ['s4-flash'],      # Vibe Coding Tools -> flashcards
    # Module 5: match on use cases, drag-drop on solution design
    'm5-s3': ['s5-drag'],       # Solution Design Process -> drag-drop
    'm5-s4': ['s5-match'],      # Operational Coordination -> match (use cases across categories)
}


# ══════════════════════════════════════════════════════════════
# COURSE CONTENT - All 5 modules (NO videos, NO quizzes)
# ══════════════════════════════════════════════════════════════
    ...
    # ── MODULE 1: Introduction to AI Automation ──
    {
        ...
        'slides': [
            ...
            },
            {
                ...
                'content_ar': '<div class="info-box violet"><strong>أتمتة العمليات</strong> تربط خطوات آلية متعددة في سير عمل موحد.</div><div class="info-box cyan" style="margin-top:12px"><strong>الخصائص:</strong><ul style="margin:8px 0 0 16px"><li>خطوات متعددة مترابطة بالتتابع</li><li>قد تتضمن شروطاً وتفرعات</li><li>تتدفق البيانات من خطوة إلى التالية</li><li>تقلل التسليمات بين الأنظمة والأشخاص</li></ul></div>',
            },
            {
                'id': 'm1-s8',
...
.flash-inner{position:relative;width:100%;min-height:80px;transition:transform .5s;transform-style:preserve-3d}
.flash-card.flipped .flash-inner{transform:rotateY(180deg)}
.flash-front,.flash-back{position:absolute;top:0;left:0;width:100%;min-height:80px;backface-visibility:hidden;border-radius:var(--radius);padding:14px;font-size:13px;display:flex;align-items:center;justify-content:center;text-align:center}
...
# ══════════════════════════════════════════════════════════════
# ACTIVITY JS BUILDERS
# ══════════════════════════════════════════════════════════════

def build_match_js(activity, idx):
    """Build matching activity HTML+JS."""
...
def build_flashcard_js(activity, idx):
    """Build flashcard activity HTML+JS."""
...
(function(){{
  ...
  function render(){{
    ...
    const el=document.getElementById('{act_id}_card');
...
def build_dragdrop_js(activity, idx):
    """Build drag-drop ordering activity HTML+JS."""
...
def build_activity_html(slide_id):
    """Build all activity HTML for a given slide."""
    act_ids = SLIDE_ACTIVITIES.get(slide_id, [])
    if not act_ids:
        return ''
    html = ''
    for idx, act_id in enumerate(act_ids):
        activity = ACTIVITIES.get(act_id)
        if not activity:
            continue
        if activity['type'] == 'match':
            html += build_match_js(activity, idx)
        elif activity['type'] == 'flashcard':
            html += build_flashcard_js(activity, idx)
        elif activity['type'] == 'dragdrop':
            html += build_dragdrop_js(activity, idx)
    return html
...
def build_slide_html(module, slide, slide_idx):
    ...
    # Build activities
    activity_html = build_activity_html(slide['id'])
    has_activity = bool(activity_html)
...
'''
    ...
    # Image
    if image_b64:
        html += f'      <div style="max-width:500px;margin:12px auto"><img src="{image_b64}" alt="{s_title}"></div>\n'
...
'''
    if has_activity:
        html += f'      <span class="badge badge-cyan">Interactive Activity</span>\n'
...
# ══════════════════════════════════════════════════════════════
# Open edX course export builder
# ══════════════════════════════════════════════════════════════

def build_openedx_tar(module, slide, slide_idx, zip_path, zip_name):
    ...
    folder_name = f"m{mod_num}_topic{slide_idx+1}"
    
    display_name = f"Module {mod_num} - Topic {slide_idx+1}: {s_title}"
    
    # XML files
    course_root = f'<course url_name="course" org="HealthAI" course="{course_id}" />'
    course_xml = f'''<course display_name="{display_name}" language="en" start="2026-01-01T00:00:00Z">
  ...
  <iframe src="/static/{folder_name}/index.html" width="100%" height="800" frameborder="0" allowfullscreen="allowfullscreen"></iframe>
    ...
    grading = json.dumps({"GRADER": [{"type": "Module Quiz", "min_count": 5, "drop_count": 0, "weight": 1.0}], "GRADE_CUTOFFS": {"Pass": 0.7}})
    
    tar_name = f"m{mod_num}-topic{slide_idx+1}-{s_id}-openedx.tar.gz"
    tar_dir = os.path.join(OUTPUT_DIR, f'module{mod_num}')
    ...
    with tarfile.open(tar_path, 'w:gz') as tar:
        ...
        # Extract index.html from zip and add to static folder
        with zipfile.ZipFile(zip_path, 'r') as zf:
            idx_html = zf.read('index.html')
            info = tarfile.TarInfo(name=f'static/{folder_name}/index.html')
            info.size = len(idx_html)
            tar.addfile(info, io.BytesIO(idx_html))
...
def build_packages():
    ...
    for module in MODULES:
        ...
        for slide_idx, slide in enumerate(module['slides']):
            ...
            # Build HTML (no quiz, no video, no menu)
            html = build_slide_html(module, slide, slide_idx)
            ...
            # Build Open edX tar.gz
            tar_path, tar_name = build_openedx_tar(module, slide, slide_idx, zip_path, zip_name)
            
            size_kb = os.path.getsize(zip_path) / 1024
            tar_kb = os.path.getsize(tar_path) / 1024
            has_act = bool(SLIDE_ACTIVITIES.get(s_id))
            has_img = bool(slide.get('image_key'))
            act_icon = '🎯' if has_act else '  '
            img_icon = '🖼️' if has_img else '  '
            print(f"  {img_icon}{act_icon} Topic {slide_idx + 1}: {s_title:45s} zip={size_kb:.1f}KB  tar={tar_kb:.1f}KB")
...
def build_archives(all_packages):
    """Build combined archives."""
    # SCORM zips archive
    ...
    print(f"\n📦 SCORM zips archive: all-scorm-zips.tar.gz ({z_kb:.1f} KB)")
...
print("\n🏗️  Building clean SCORM packages (no videos, no quizzes, no menu)...")
all_packages = build_packages()

print(f"\n📊 Summary:")
total = len(all_packages)
total_zip = sum(p['size_kb'] for p in all_packages)
total_tar = sum(p['tar_kb'] for p in all_packages)
act_count = sum(1 for p in all_packages if p['has_activity'])
img_count = sum(1 for p in all_packages if p['has_image'])
print(f"  Total packages: {total}")
print(f"  Total SCORM zip size: {total_zip:.1f} KB")
print(f"  Total Open edX tar size: {total_tar:.1f} KB")
print(f"  With activities: {act_count}")
...
for pkg in all_packages:
    if pkg['module'] != current_mod:
        ...
        print(f"  {'─'*60}")
    a = '🎯' if pkg['has_activity'] else '  '
    print(f"    {a} Topic {pkg['topic']:2d}: {pkg['title']}")

build_archives(all_packages)

print(f"\n✅ All {total} clean SCORM packages built!")
print(f"📁 Output: {OUTPUT_DIR}/")
print(f"   Each module folder has both .zip (SCORM) and .tar.gz (Open edX) files")
print(f"\n🔧 What's included per slide:")
print(f"   ✅ Slide content (bilingual EN/AR)")
print(f"   ✅ Interactive activities (where applicable)")
print(f"   ✅ Images (base64-embedded)")
print(f"   ✅ SCORM 1.2 API integration")
print(f"   ✅ imsmanifest.xml")
print(f"\n❌ Excluded:")
print(f"   ❌ No videos")
print(f"   ❌ No quizzes")
print(f"   ❌ No topic menu/sidebar")
